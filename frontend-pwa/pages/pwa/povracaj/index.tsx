...
import { getMe, getPovracajSummary, listPovracajDocuments } from '../../../src/lib/apiClient';

export default function PovracajHomePage() {
  ...
  useEffect(() => {
    ...
    getPovracajSummary('today').then(setSummary).catch((e) => {
      setToast(e?.message || 'Neuspešno učitavanje Povraćaj statistike.');
    });
    loadDocuments();
  }, [router]);
  ...
  async function loadDocuments() {
    try {
      setLoadingDocs(true);
      const response = await listPovracajDocuments({ status: 'ALL' });
  ...
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(180deg, #05070d 0%, #020304 100%)', color: '#fff' }}>
      ...
      <div style={{ padding: '16px' }}>
        ...
        <div style={{ fontSize: 'clamp(1.5rem, 4vw, 2rem)', fontWeight: 800, marginBottom: 8, background: 'linear-gradient(135deg,#ffd400 0%,#ffaa00 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', letterSpacing: '-0.02em' }}>Povraćaj modul</div>
        <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.6)', marginBottom: 24, lineHeight: 1.6 }}>
          Digitalno evidentiraj povratnu robu – skeniraj dokument, dodaj fotografije i završi predaju.
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {canCreate && (
            <button
              onClick={() => router.push('/pwa/povracaj/create')}
              style={{
                width: '100%',
          ...
          <button
            onClick={() => router.push('/pwa/povracaj/scan')}
            style={{
              width: '100%',
        ...
        <div style={{ marginTop: 32 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div style={{ fontSize: 18, fontWeight: 700, color: '#fde68a' }}>Moji Povraćaj dokumenti</div>
            <button
          ...
          {loadingDocs ? (
            ...
            <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: 14, padding: 20, textAlign: 'center' }}>Još uvek nema Povraćaj prijava za tvoju prodavnicu.</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {documents.slice(0, 8).map((doc: any) => (
                <button
                  key={doc.uid}
                  onClick={() => router.push(`/pwa/povracaj/${encodeURIComponent(doc.uid)}`)}
                  style={{
