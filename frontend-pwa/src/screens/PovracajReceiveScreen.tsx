...
import { getMe, getPovracajDocumentByUid, receivePovracajDocument, uploadPovracajDocumentPhoto } from '../lib/apiClient';
...
export default function PovracajReceiveScreen({ uid }: { uid: string }) {
  ...
  async function loadDocument() {
    try {
      setLoading(true);
      const response = await getPovracajDocumentByUid(uid);
      setDoc(response);
      ...
      setToast(e?.message || 'Neuspešno učitavanje POVRAĆAJ dokumenta.');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleFinish() {
    ...
    try {
      setLoading(true);
      await receivePovracajDocument(uid, {
        note: note || undefined,
  ...
  async function handleUploadPhoto(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    setToast(null);
    try {
      await uploadPovracajDocumentPhoto(doc.uid, files[0]);
      await loadDocument();
      ...
      <div className="min-h-screen" style={{ background: '#000', color: '#fff' }}>
        ...
        <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ textAlign: 'center', background: '#14532d', borderRadius: 16, padding: 24, border: '2px solid #22c55e' }}>
            <div style={{ fontSize: 24, fontWeight: 800, marginBottom: 12, color: '#22c55e' }}>Povracaj primljen uspešno!</div>
    ...
    <div className="min-h-screen" style={{ background: '#000', color: '#fff' }}>
      ...
      <div style={{ padding: 16 }}>
        ...
        <div style={{ border: '2px solid #FFC300', borderRadius: 16, padding: 16, marginBottom: 16 }}>
          ...
          <div style={{ color: '#ccc', marginBottom: 4 }}>Status: {translatePovracajStatus(doc.status)}</div>
        ...
        <div style={{ background: '#111', borderRadius: 16, padding: 16, marginBottom: 20 }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: '#FFC300', marginBottom: 12 }}>Dokaži povracaj</div>
          {canReceive ? (
            <label
              style={{
                display: 'flex',
            ...
            <div style={{ display: 'grid', gap: 12 }}>
              {doc.photos.map((photo: any) => (
                <a key={photo.id} href={photo.path} target="_blank" rel="noreferrer" style={{ border: '1px solid #333', borderRadius: 12, overflow: 'hidden' }}>
                  <img src={photo.path} alt="POVRAĆAJ" style={{ width: '100%', height: 160, objectFit: 'cover' }} />
