...
const PovracajCreateScreen = () => {
    ...
    async (index: number, code: string) => {
      ...
      try {
        ...
        if (!selected) {
          ...
          logger.debug('No exact match for code', { trimmed, resultsCount: results.length }, 'PovracajCreate');
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        scanner = new Html5QrcodeScanner('povracaj-item-scanner', { fps: 10, qrbox: 220 }, false);
        scanner.render(
          (decodedText: string) => {
  ...
  async function handleSubmit() {
    ...
    if (itemsWithoutPhotos.length > 0) {
      ...
      setToast(`Dodajte fotografiju povraćaja za artikal(e) #${itemIndexes.join(', ')}. Fotografija je obavezna za svaki artikal.`);
      return;
    }
    setToast(null);
    setLoading(true);
    try {
      const response = await createPovracajDocument({
        storeId: me.store_id,
      ...
      <div className="min-h-screen" style={{ background: '#000', color: '#fff' }}>
        ...
        <div style={{ padding: 16 }}>
          ...
          <div style={{ marginTop: 40, textAlign: 'center', color: '#f87171', fontWeight: 700 }}>
            Nemate dozvolu za kreiranje Povraćaj naloga.
          </div>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen" style={{ background: '#000', color: '#fff' }}>
        ...
        <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: '#14532d', border: '2px solid #22c55e', borderRadius: 18, padding: 24, textAlign: 'center' }}>
            <div style={{ fontSize: 22, fontWeight: 800, color: '#22c55e', marginBottom: 12 }}>Povraćaj nalog je kreiran!</div>
            ...
            <button
              onClick={() => router.push(`/pwa/povracaj/${encodeURIComponent(successDoc.uid)}`)}
  ...
  return (
    <div className="min-h-screen" style={{ background: '#000', color: '#fff' }}>
      ...
      <div style={{ padding: 16, paddingBottom: 32 }}>
        ...
        <div style={{ fontSize: 24, fontWeight: 800, color: '#FFC300', marginBottom: 8 }}>Novi Povraćaj nalog</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Opis razloga povraćaja"
                    style={textInputStyle}
                  />
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: item.photos?.length ? '#10b981' : '#f87171', marginBottom: 8, fontWeight: item.photos?.length ? 'normal' : 'bold' }}>
                    {item.photos?.length 
                      ? `✓ Dodate fotografije povraćaja (${item.photos.length})`
                      : '⚠ Dodajte fotografiju povraćaja koja pokazuje stanje ovog artikla (obavezno).'}
                  </div>
        ...
        >
          {loading ? 'Kreiranje...' : 'Kreiraj Povraćaj nalog'}
        </button>
      </div>
        ...
        >
          <div style={{ width: '100%', maxWidth: 600, background: '#000', borderRadius: 18, padding: 16, border: '1px solid #444' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <div style={{ color: '#FFC300', fontSize: 18, fontWeight: 700 }}>Kamera - Slikaj povraćaj</div>
              <button
                onClick={stopCamera}
                style={{
                  background: '#dc2626',
      ...
      {scannerIndex !== null && (
        ...
        >
          <div style={{ width: '100%', maxWidth: 420, background: '#000', borderRadius: 18, padding: 16, border: '1px solid #444' }}>
            ...
            <div id="povracaj-item-scanner" style={{ width: '100%', minHeight: 260, background: '#111', borderRadius: 12 }} />
...
export default PovracajCreateScreen;
