...
import { PovracajDocument, PovracajStatus } from './entities/povracaj-document.entity';
...
import { CreatePovracajDto } from './dto/create-povracaj.dto';
import { ReceivePovracajDto } from './dto/receive-povracaj.dto';
import { randomBytes } from 'crypto';
import { promises as fs } from 'fs';
import { join } from 'path';
import { buildPovracajQrPdf } from '../common/pdf/qr.util';

type Actor = { id: number; role: string; name?: string; storeId?: number | null };

@Injectable()
export class PovracajService {
  private readonly uploadsRoot = join(process.cwd(), 'uploads', 'povracaj');
  private readonly incrementInventory = true;

  constructor(
    @InjectRepository(PovracajDocument) private readonly documentRepo: Repository<PovracajDocument>,
    @InjectRepository(PovracajItem) private readonly itemRepo: Repository<PovracajItem>,
    @InjectRepository(PovracajPhoto) private readonly photoRepo: Repository<PovracajPhoto>,
  ...
  async listDocuments(params: { limit?: number; offset?: number; status?: PovracajStatus | 'ALL'; assignedToUserId?: number } = {}, actor?: Actor) {
  ...
  async createDocument(actor: Actor, dto: CreatePovracajDto) {
    this.ensureRole(actor.role, ['admin', 'store', 'prodavnica', 'menadzer', 'sef', 'sef_prodavnice']);
    if (!dto.items || !dto.items.length) {
      throw new BadRequestException('Dodajte najmanje jednu stavku povraćaja.');
    }
    ...
    if (actorStoreId && requestedStoreId && requestedStoreId !== actorStoreId) {
      throw new ForbiddenException('Niste ovlašćeni da kreirate Povraćaj za drugu prodavnicu.');
    }
    ...
    // Validacija: svaki artikal mora imati bar jednu fotografiju
    dto.items.forEach((item, index) => {
      if (!item.reason || !item.reason.trim()) {
        throw new BadRequestException(`Unesite razlog povraćaja za stavku #${index + 1}.`);
      }
      if (Number(item.qty) <= 0) {
        throw new BadRequestException(`Količina mora biti veća od 0 za stavku #${index + 1}.`);
      }
      if (!item.photos || !Array.isArray(item.photos) || item.photos.length === 0) {
        throw new BadRequestException(`Fotografija je obavezna za artikal "${item.name || item.code}" (stavka #${index + 1}). Dodajte bar jednu fotografiju.`);
      }
    });
  ...
  async getDocumentByUid(uid: string, actor?: Actor) {
    ...
    if (!row) throw new NotFoundException('Povraćaj dokument nije pronađen.');
    if (actor && this.isStoreScoped(actor.role)) {
      const storeId = actor.storeId ?? null;
  ...
  async receiveDocument(actor: Actor, uid: string, dto: ReceivePovracajDto) {
    this.ensureRole(actor.role, ['admin', 'magacioner', 'warehouse', 'sef', 'menadzer']);
    const document = await this.documentRepo.findOne({
      where: { uid },
      relations: ['items', 'store'],
    });
    if (!document) throw new NotFoundException('Povraćaj dokument nije pronađen.');
    if (document.status === PovracajStatus.RECEIVED) {
    ...
    document.status = PovracajStatus.RECEIVED;
  ...
  async assignDocument(actor: Actor, uid: string, assignedToUserId: number | null) {
    ...
    if (!document) {
      throw new NotFoundException('Povraćaj dokument nije pronađen.');
    }
    ...
    if (assignedToUserId !== null) {
      ...
      if (!allowedRoles.includes(assignedUser.role?.toLowerCase() || '')) {
        throw new BadRequestException('Korisnik mora biti magacioner da bi mogao da primi Povraćaj nalog.');
      }
    }
  ...
  async summaryReport(params: { from?: string; to?: string; storeId?: number; window?: string } = {}, actor?: Actor) {
    ...
    for (const doc of documents) {
      submittedCount += 1;
      if (doc.status === PovracajStatus.RECEIVED) receivedCount += 1;
      ...
      if (doc.status === PovracajStatus.RECEIVED) storeStat.received += 1;
      ...
      dayStat.submitted += 1;
      if (doc.status === PovracajStatus.RECEIVED) dayStat.received += 1;
  ...
  async generateQrPdf(uid: string, actor?: Actor) {
    ...
    if (!doc) throw new NotFoundException('Povraćaj dokument nije pronađen.');
    if (actor && this.isStoreScoped(actor.role)) {
      const storeId = actor.storeId ?? null;
    ...
    return buildPovracajQrPdf(payload);
  }
    ...
    try {
      for (const mv of movements) {
        await queryRunner.manager.query(
          ...
          [mv.itemId, null, null, Math.abs(mv.qty), 'POVRACAJ', documentId, actorId],
        );

        if (this.incrementInventory) {
          // Za Povraćaj, inkrementujemo inventar (roba se vraća u magacin)
          const inventoryRepo = queryRunner.manager.getRepository(Inventory);
          // Traži postojuću inventar stavku ili kreiraj novu
          let inventoryRow = await inventoryRepo.findOne({
            where: { item_id: mv.itemId },
          });
          if (inventoryRow) {
            const current = Number(inventoryRow.quantity as any) || 0;
            inventoryRow.quantity = this.toNumericString(current + mv.qty);
            await inventoryRepo.save(inventoryRow);
          } else {
            // Ako ne postoji, kreiraj novu (primeti: možda treba location_id, ali za sada ću ostaviti null)
            inventoryRow = inventoryRepo.create({
              item_id: mv.itemId,
              quantity: this.toNumericString(mv.qty),
              location_id: null, // Možda treba dodati lokaciju, ali za sada null
            });
            await inventoryRepo.save(inventoryRow);
          }
        }
  ...
  private async persistBase64Photos(documentId: number, uid: string, photos: string[], actorId: number, itemId?: number | null) {
    ...
    for (const raw of photos) {
      try {
        ...
        const webPath = `/uploads/povracaj/${uid}/${fileName}`;
        records.push(
  ...
  private buildDocumentResponse(document: PovracajDocument) {
    return {
      id: document.id,
      uid: document.uid,
  ...
  private generateUid() {
    return `PR-${randomBytes(4).toString('hex').toUpperCase()}`;
  }
