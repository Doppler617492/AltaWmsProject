...
@Controller('povracaj')
export class PovracajController {
  constructor(private readonly povracajService: PovracajService) {}
  ...
  async listDocuments(
    ...
    return this.povracajService.listDocuments({
  ...
  @Post()
  async createDocument(@Body() dto: CreatePovracajDto, @Req() req: any) {
    ensureRole(req.user?.role, ['admin', 'store', 'prodavnica', 'menadzer', 'sef', 'sef_prodavnice']);
    return this.povracajService.createDocument(
      { id: req.user?.id, role: req.user?.role, name: req.user?.name, storeId: req.user?.store_id ?? null },
      dto,
    );
  }
  ...
  async getDocument(@Param('uid') uid: string, @Req() req: any) {
    ensureRole(req.user?.role, ['admin', 'menadzer', 'sef', 'magacioner', 'store', 'prodavnica', 'sef_prodavnice']);
    return this.povracajService.getDocumentByUid(uid, { id: req.user?.id, role: req.user?.role, name: req.user?.name, storeId: req.user?.store_id ?? null });
  }

  @Patch(':uid/receive')
  async receiveDocument(@Param('uid') uid: string, @Body() dto: ReceivePovracajDto, @Req() req: any) {
    ensureRole(req.user?.role, ['admin', 'magacioner', 'warehouse', 'sef', 'menadzer']);
    return this.povracajService.receiveDocument(
      { id: req.user?.id, role: req.user?.role, name: req.user?.name, storeId: req.user?.store_id ?? null },
      uid,
      dto,
    );
    ...
    FileInterceptor('file', {
      storage: diskStorage({
        destination: (_req, _file, cb) => {
          const dest = join(process.cwd(), 'uploads', 'povracaj-temp');
  ...
  async uploadPhoto(
    ...
    return this.povracajService.addPhoto(
      { id: req.user?.id, role: req.user?.role, name: req.user?.name, storeId: req.user?.store_id ?? null },
      uid,
      webPath,
    );
  ...
  async summary(
    ...
    return this.povracajService.summaryReport({
      from,
      to,
  ...
  async anomalies(
    ...
    ensureRole(req.user?.role, ['admin', 'menadzer', 'manager', 'sef', 'magacioner', 'store', 'prodavnica', 'sef_prodavnice']);
    return this.povracajService.anomaliesReport({ from, to, window }, { id: req.user?.id, role: req.user?.role, name: req.user?.name, storeId: req.user?.store_id ?? null });
  }

  @Get('qr/:uid/pdf')
  async downloadPdf(@Param('uid') uid: string, @Res() res: Response, @Req() req: any) {
    ensureRole(req.user?.role, ['admin', 'menadzer', 'manager', 'sef', 'magacioner', 'store', 'prodavnica', 'sef_prodavnice']);
    const buffer = await this.povracajService.generateQrPdf(uid, { id: req.user?.id, role: req.user?.role, name: req.user?.name, storeId: req.user?.store_id ?? null });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `inline; filename="POVRACAJ-${uid}.pdf"`);
    res.send(buffer);
  }
  ...
  async assignDocument(@Param('uid') uid: string, @Body() body: { assignedToUserId?: number | null }, @Req() req: any) {
    ...
    return this.povracajService.assignDocument(
      { id: req.user?.id, role: req.user?.role, name: req.user?.name, storeId: req.user?.store_id ?? null },
      uid,
  ...
  async deleteDocument(@Param('uid') uid: string, @Req() req: any) {
    ensureRole(req.user?.role, ['admin', 'menadzer', 'sef', 'sef_magacina']);
    await this.povracajService.deleteDocument(
      { id: req.user?.id, role: req.user?.role, name: req.user?.name, storeId: req.user?.store_id ?? null },
      uid,
    );
    return { success: true, message: 'Povraćaj dokument je obrisan.' };
  }

  private async movePhotoToDocumentFolder(uid: string, file: any) {
    const targetDir = join(process.cwd(), 'uploads', 'povracaj', uid);
    await fs.mkdir(targetDir, { recursive: true });
    const destination = join(targetDir, file.filename);
    await fs.rename(file.path, destination);
    return `/uploads/povracaj/${uid}/${file.filename}`;
  }
}
