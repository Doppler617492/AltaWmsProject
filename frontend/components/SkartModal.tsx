...
export default function SkartModal({ mode, document, onClose, onSaved, canReceive, canCreate, canDelete = false, canAssign = false, userRole }: SkartModalProps) {
  ...
  async function handleAssign(assignedToUserId: number | null) {
    ...
    setError(null);
    try {
      await apiClient.assignSkartDocument(activeDoc.uid, assignedToUserId);
  ...
  function handlePrintDocument() {
    ...
    const statusLabel = translateSkartStatus(activeDoc.status);
    ...
    const content = `
      ...
      <html>
        <head>
          <title>SKART dokument - ${activeDoc.uid}</title>
          <style>
            @media print {
              @page { margin: 0.5cm; }
              body { margin: 0; }
        ...
        <body>
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th style="width: 15%;">Razlog SKART-a</th>
          ...
          <div class="totals">
            ...
            <strong>Ukupno količina (SKART): ${totalQty.toLocaleString('sr-Latn-RS')} kom</strong>
            ${activeDoc.status === 'RECEIVED' ? `<strong>Ukupno primljeno: ${totalReceived.toLocaleString('sr-Latn-RS')} kom</strong>` : ''}
            <strong>Razlozi SKART-a: ${[...new Set((activeDoc.items || []).map((i: any) => i.reason).filter(Boolean))].join(', ')}</strong>
          </div>
  ...
  async function refreshDocument() {
    ...
    try {
      setIsLoading(true);
      const fresh = await apiClient.getSkartDocument(document.uid);
      setActiveDoc(fresh);
  ...
  async function handleCreate() {
    if (!canCreate) {
      setError('Nemate dozvolu za kreiranje SKART dokumenta.');
      return;
    }
    if (!form.storeId) {
    ...
    setIsLoading(true);
    try {
      await apiClient.createSkartDocument({
  ...
  async function handleReceive() {
    ...
    setIsLoading(true);
    try {
      await apiClient.receiveSkart(activeDoc.uid, {
        items: payload,
        note: form.note || undefined,
      });
      await refreshDocument();
  ...
  async function handlePhotoUpload(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    setPhotoError(null);
    try {
      await apiClient.uploadSkartPhoto(activeDoc.uid, files[0]);
      await refreshDocument();
    } catch (e: any) {
  ...
  const modalTitle = mode === 'create' ? 'Novi SKART dokument' : `SKART ${activeDoc?.uid || ''}`;
            ...
            {mode === 'view' && activeDoc && (
              <div style={styles.badgeRow}>
                <span style={{ ...styles.badge, background: activeDoc.status === 'RECEIVED' ? '#15803d' : '#ca8a04' }}>
                  {translateSkartStatus(activeDoc.status)}
                </span>
          ...
          <>
            ...
            <div style={styles.footer}>
              <button style={styles.primaryBtn} onClick={handleCreate} disabled={isLoading || !canCreate}>
                {isLoading ? 'Čuvanje...' : 'Kreiraj SKART'}
              </button>
              ...
              <div style={{ display: 'flex', gap: 12, flex: 1 }}>
                ...
                {activeDoc.status === 'SUBMITTED' && canReceive && (
                  <button style={styles.primaryBtn} onClick={handleReceive} disabled={isLoading}>
                    {isLoading ? 'Obrada...' : 'Završi prijem SKART-a'}
                  </button>
                )}
                {canDelete && (
                  <button
                    ...
                    onClick={async () => {
                      if (!window.confirm('Da li ste sigurni da želite da obrišete ovaj SKART dokument? Ova akcija je nepovratna.')) {
                        return;
                      }
                      setIsLoading(true);
                      setError(null);
                      try {
                        await apiClient.deleteSkartDocument(activeDoc.uid);
                        onSaved();
                        onClose();
                      } catch (e: any) {
