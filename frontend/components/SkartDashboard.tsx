...
import SkartModal from './SkartModal';
import { apiClient } from '../lib/apiClient';

interface SkartDashboardProps {
  user: any;
}
...
export default function SkartDashboard({ user }: SkartDashboardProps) {
  ...
  const canCreate = ['admin', 'store', 'prodavnica', 'menadzer', 'sef', 'sef_magacina'].includes(user?.role);
  ...
  async function refreshData() {
    ...
    try {
      const [docs, sum, anomaly] = await Promise.all([
        apiClient.getSkartDocuments({ status: 'ALL', limit: 50 }),
        apiClient.getSkartSummary({ window: 'today' }),
        apiClient.getSkartAnomalies({ window: 'today' }),
      ]);
      setDocuments(Array.isArray(docs?.data) ? docs.data : []);
      setSummary(sum);
      setAnomalies(Array.isArray(anomaly) ? anomaly : []);
    } catch (e: any) {
      setError(e?.message || 'Neuspešno učitavanje SKART podataka.');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleDownload(uid: string) {
    try {
      const blob = await apiClient.downloadSkartPdf(uid);
  ...
  function exportCsv(doc: any) {
    ...
    link.download = `SKART_${doc.uid}.csv`;
    link.click();
  ...
  async function handleDelete(uid: string) {
    ...
    setError(null);
    try {
      await apiClient.deleteSkartDocument(uid);
      setError(null);
      await refreshData();
    } catch (e: any) {
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <div>
          <h1 style={styles.title}>Skart nadzor</h1>
      ...
      {anomalies.length > 0 && (
        <div style={styles.warning}>
          <strong>Upozorenje:</strong> {anomalies.map((a) => `${a.storeName} (${Math.round(a.share * 100)}%)`).join(', ')} prelaze prag od 30% skarta za izabrani period.
        </div>
      )}
          ...
          <table style={styles.table}>
            ...
            <tbody>
                ...
                <tr key={doc.id}>
                  ...
                  <td style={styles.td}>
                    <span style={{ ...styles.statusBadge, background: doc.status === 'RECEIVED' ? '#15803d' : '#ca8a04' }}>
                      {translateSkartStatus(doc.status)}
                    </span>
                  </td>
              ...
              {documents.length === 0 && !loading && (
                <tr>
                  <td colSpan={6} style={styles.emptyState}>Nema dostupnih SKART dokumenata.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <div style={styles.analyticsGrid}>
        <div style={styles.analyticsCard}>
          <h3 style={styles.sectionTitle}>Skart po prodavnici</h3>
        ...
        <div style={styles.analyticsCard}>
          <h3 style={styles.sectionTitle}>Skart po razlogu</h3>
          <div style={styles.list}>
